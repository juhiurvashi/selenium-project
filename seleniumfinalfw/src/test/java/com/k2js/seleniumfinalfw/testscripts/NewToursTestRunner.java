package com.k2js.seleniumfinalfw.testscripts;
...
public class NewToursTestRunner {
	...
	public void verifyHomepage(String... td) throws IOException {
		...
		// String expTitle=CommonUtil.getPropertyValue("home", "exp_title");
		
		 * if(actualTitle.equalsIgnoreCase(expTitle)) { System.out.println("Pass"); }
		 * else { System.out.println("Fail"); }
		 
		Assert.assertEquals(title, actualTitle);
		Assert.assertTrue(actualTitle.equalsIgnoreCase(title));
		Assert.assertEquals(title, actualTitle, "Title not Valid");*/
		...
		try {
			String actualTitle = hp.getHomepagetitle();
			Assert.assertEquals(td[0], actualTitle);
		}
		catch(Throwable e)
		{
			result="fail";
	...
	public void verifyRegisterPage(String... td) throws Throwable {
		...
		try {
			...
			System.out.println("expected msg is : "+expTitle.trim() );
			System.out.println("actucal msge is : "+actualTitle);
			

			Assert.assertEquals(actualTitle.trim(), expTitle.trim(), "Title not Valid");
